...
const updateLocationAndFetchNearbyUsers = asyncHandler(async (req, res) => {
    ...
    // Check that the user has valid location and radius
    if (!user.currentLocation || !user.locationRadiusPreference || !user.currentLocation.coordinates) {
        console.log("Location or radius preference is missing!");
        throw new ApiError(400, "Location or radius preference not available");
    }
    ...
    // Define 15-minute threshold for lastActive status
    const fifteenMinutesAgo = new Date(Date.now() - 15 * 60 * 1000); // 15 minutes ago

    // Find nearby users within the location radius and ensure they are active OR were active in the last 15 min
    const nearbyUsers = await User.find({
        currentLocation: {
            $geoWithin: {
                $centerSphere: [
        ...
        _id: { $ne: userId }, // Exclude the current user from the result
        $or: [
            { "activeStatus.isActive": true }, // Include users who are currently active
            { "activeStatus.lastActive": { $gte: fifteenMinutesAgo } } // Include users who were active in the last 15 minutes
        ]
